...
const duplicateLoc = (id: string) => setLocations((prev) => { const t = prev.find((l) => l.id === id); if (!t) return prev; const copy = { ...t, id: (typeof crypto !== 'undefined' && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : Math.random().toString(36).slice(2), name: t.name + " (copy)" }; return [...prev, copy]; });
